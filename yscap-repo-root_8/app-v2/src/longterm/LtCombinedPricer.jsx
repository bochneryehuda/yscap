...
/**
 * THE COMBINED PRICING ENGINE — every rate BOTH programs are quoting, and every investor at each one.
 *
 * ⛔ THIS FILE IS A DELIBERATE, OWNER-DIRECTED FORK OF `LtPricer.jsx`, AND IT IS TEMPORARY.
 *
 * Owner-directed 2026-08-30, twice and in these words:
 *
 ...
 *    is. Combining them, I am going to test the system that works on both together: Lender Price
 *    and LoanNEX. If it's going to be good, then I am going to merge everything into the General
 *    Pricing Engine and bring in the features of this one."
 *
 * So this is a COPY, on purpose, against this repository's own one-definition rule — because the
 * owner is auditing a second engine beside the first and the first must not move while they do.
 ...
 *
 * ⛔ SUPER ADMIN ONLY. The server answers 404 to anybody else (routes/combined-pricer.js), and the
 * nav entry is hidden. The owner is auditing it privately before it reaches the general engine.
 *
 * ── the general engine's own header follows, and still describes this screen ──
 *
  ...
  list.forEach((p, pi) => {
    (Array.isArray(p.options) ? p.options : []).forEach((o, oi) => {
      ...
      const entry = {
        ...
        // been stale. So an unknown is carried as its own fact and drawn as an em dash below.
        ...
        <div style={{ padding: '0 14px 12px' }}>
          <div className="ltq-head" style={{ display: 'flex', gap: 10, padding: '6px 0', borderBottom: `1px solid ${GOLD}44`, fontSize: 10.5, letterSpacing: '.07em', textTransform: 'uppercase', color: MUTED, fontWeight: 700 }}>
            ...
            {/* THE COLUMN NAMES ITS LENS (design research 2026-08-23: the comp switch silently
                ...
                On a phone this heading row is hidden and the SAME words ride on the cell
                itself (`data-k`), so both must come from `priceKey` or the two surfaces
                could disagree about which position is on screen. */}
...
export default function LtCombinedPricer() {
  ...
  /* FORK 4 of 8 — the admin's "click to see the source of the info". OFF by default, because the
     ...
     un-mask. */
  const [revealSrc, setRevealSrc] = useState(false);
  ...
  useEffect(() => {
    ...
    ltApi.combinedInvestors()
      .then((r) => { if (live) { setInvRoster(((r && r.investors) || []).map((x) => ({ key: x.key, investor: x.label, whiteLabel: x.whiteLabel }))); setInvRosterStatus('ok'); } })
  ...
  async function run(e, opts) {
    ...
    // Read the reveal from the CALLER when it is passing one. A checkbox handler that calls
    // setState and then run() in the same tick would otherwise price the value being replaced,
    // and the board would come back without the sources the person just asked for.
    const reveal = opts && opts.reveal !== undefined ? !!opts.reveal : revealSrc;
  ...
  return (
    <LtLayout title="Combined Pricing Engine">
      {/* FORK 5 of 8 — SAY WHAT THIS SCREEN IS, at the top, before anything else. The owner is
          ...
          one that is still under audit. It is a plain statement, not a warning colour: this board
          is not broken, it is simply not the one the company prices on yet. */}
      <div style={{ ...card, borderColor: `${GOLD}66`, background: '#FFFDF7' }}>
        <div style={eyebrow}>Under audit — not the company&rsquo;s pricing engine yet</div>
        <div style={{ fontSize: 13, color: SLATE, marginTop: 6, lineHeight: 1.7 }}>
          This is a SECOND engine, beside the General Pricing Engine. It prices the same loan on{' '}
          <strong style={{ color: INK }}>both</strong> programs and shows one board. The General
          Pricing Engine is unchanged and is still the one the company prices on.
        </div>
      </div>
