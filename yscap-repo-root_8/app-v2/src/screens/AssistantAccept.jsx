    ...
    return (
      <AuthShell title="Invitation missing" subtitle="This page needs a valid helper invitation link.">
        ...
        <div className="row" style={{ marginTop: 16 }}>
          <button className="btn primary" onClick={() => nav('/assistant/login')}>Go to helper sign in</button>
        </div>
      </AuthShell>
    );
  ...
  return (
    <AuthShell title="Set up your helper login"
      ...
      <div className="row" style={{ marginTop: 8 }}>
        <button className="btn primary" disabled={busy || !pw || !pw2} onClick={submit}>Activate helper login</button>
        <div className="spacer" />
        <button className="btn link" onClick={() => nav('/assistant/login')}>Sign in instead</button>
      </div>
    </AuthShell>
  );
}
