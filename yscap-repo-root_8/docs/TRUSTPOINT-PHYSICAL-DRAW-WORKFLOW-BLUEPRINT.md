...
**E. Retainage & fees precedence (D6/D7):** TrustPoint is the retainage system of record — PILOT mirrors its 4 draw fields and **disables its own `retainage_pct` computation** on these files (double-withholding risk). Fees: the **$250 Blue Lake physical fee is deducted from the net wire** — PILOT mirrors the draw's `fees[]`, verifies a $250 fee line is present on every Blue Lake physical draw (staff alert if absent or different), shows NET = approved − fees to the borrower, and records the fee on the mirror ledger row (receivable/remittance bookkeeping = open question Q-A).
...
The original eight blocking questions were answered 2026-07-24 — recorded as **D1–D8 in §0**. Remaining open items:

- **Q-A (bookkeeping, non-blocking):** the $250 fee is deducted from the net wire that Blue Lake releases — does that $250 come back to YS (remittance), and should PILOT track it as a receivable per draw, or is netting handled outside the system?
