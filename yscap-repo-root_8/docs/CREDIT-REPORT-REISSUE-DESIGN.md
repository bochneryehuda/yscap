...
**1c — DB + FICO freeze** (`db/143`): provider registry, per-user encrypted credentials,
...
**1e — order/import + wiring** (`credit/import.js`, `db/144–147`,
...
compared an **individual** borrower's score, not the loan representative. `db/144` supersedes the
...
unextracted) are now parsed per bureau, stored (`db/148`), and auto-seed the adverse-action
...
status, "N of 3" chips, `db/149`).

**Compliance correctness:** an **applicant-vs-guarantor** flag (`db/150`) — a guarantor is generally
...
default; the order route takes a per-order override. `db/152` records the version per report.
