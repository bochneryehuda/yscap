...
**1c — DB + FICO freeze** (`db/173`): provider registry, per-user encrypted credentials,
...
compared an **individual** borrower's score, not the loan representative. `db/174` supersedes the
...
unextracted) are now parsed per bureau, stored (`db/178`), and auto-seed the adverse-action
...
status, "N of 3" chips, `db/179`).

**Compliance correctness:** an **applicant-vs-guarantor** flag (`db/180`) — a guarantor is generally
**not** owed an AA notice on a business-purpose loan, so the scaffold flags it rather than
over-sending.

**Observability:** an append-only `credit_order_events` black-box log (`db/181`, immutability
...
default; the order route takes a per-order override. `db/182` records the version per report.
