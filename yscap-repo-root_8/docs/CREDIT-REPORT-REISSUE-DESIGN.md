...
**1c — DB + FICO freeze** (`db/200`): provider registry, per-user encrypted credentials,
...
**1e — order/import + wiring** (`credit/import.js`, `db/201–177`,
...
compared an **individual** borrower's score, not the loan representative. `db/201` supersedes the
...
unextracted) are now parsed per bureau, stored (`db/205`), and auto-seed the adverse-action
...
status, "N of 3" chips, `db/206`).

**Compliance correctness:** an **applicant-vs-guarantor** flag (`db/207`) — a guarantor is generally
**not** owed an AA notice on a business-purpose loan, so the scaffold flags it rather than
over-sending.

**Observability:** an append-only `credit_order_events` black-box log (`db/208`, immutability
...
default; the order route takes a per-order override. `db/209` records the version per report.
