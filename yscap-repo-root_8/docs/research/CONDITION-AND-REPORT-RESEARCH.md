...
| **Cost-based** | Marshall & Swift-style replacement cost less depreciation, for the *contributory* value of a feature | **Partially, and it is under-used.** The cost approach is already parsed — `cost_new_total`, `dwelling_cost_new`, `depreciation_physical/functional/external`, `site_improvements_value` (db/422/424). It is the correct basis for a **garage, a deck, a finished basement, a bath addition** — features whose contributory value is genuinely construction cost less depreciation. It is *not* a basis for location, view, or condition |
...
| **8** | **Market conditions** | What our own sales did, with `n` and the span; the FHFA HPI for the CBSA when wired; the appraisers' own 1004MC reads from `market_observations` (db/423) **with their sample size**; days on market | db/423's own header says it: this is *"what N appraisers told us about the market around here, and when"* — **never "the market"** |
