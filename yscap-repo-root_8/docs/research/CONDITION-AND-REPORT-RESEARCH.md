...
| **Depreciation** | Physical depreciation ÷ cost new = condition expressed as a percentage | **Yes, as a CROSS-CHECK on the condition adjustment, and this is a genuinely novel use of data we already hold.** `db/450` already treats depreciation as "condition in dollars" and puts it under `AS_IS_ONLY` for exactly that reason. Two reports on similar houses at C3 and C5 with 8% and 24% physical depreciation give an independent read on what a condition grade is worth. **Subject-only** — comps carry no cost approach |
