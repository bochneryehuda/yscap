...
- **Chaim Lebowitz** (Pipeline `90118110153`) & **Mendel Bochner** (Pipeline `90118110154`) **ARE loan officers.** ✅ **DONE via connector:** CRM folders + lists created — **Chaim Lebowitz CRM** folder `90118114571` / list `901114073281`; **Mendel Bochner CRM** folder `90118114572` / list `901114073282`. Space custom fields auto-inherit (functional now). **⚠️ Still needed (UI-only, can't be API'd):** the CRM **list-view layout** + any **automations** the other officers have — set these by either (a) copying an existing officer's CRM view/automation config onto these lists, or (b) if you prefer identical automations, delete these two and instead **duplicate an existing officer's CRM folder** in the UI and rename.
...
- [ ] **Create 7 new custom fields** in the Loan Pipeline space — **UI-only** (ClickUp exposes no create-field API/connector): `Portal File ID` (text), `Send to Portal` (checkbox), `Portal File Link` (url), `Borrower Portal Status` (dropdown), `Sync Status / Last Error` (text), `Rehab Type` (dropdown), `RTL As-Is Value` (currency). Specs in `CLICKUP-DATA-MAPPING.md` Part 5. After creation I re-pull the IDs into the mapping.
- [x] **Create CRM folders + lists** for **Chaim Lebowitz** (`90118114571`/list `901114073281`) and **Mendel Bochner** (`90118114572`/list `901114073282`) — ✅ done via connector. Remaining (UI): copy the CRM view/automations onto these lists.
