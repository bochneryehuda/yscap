...
### 2.1 New tables (`db/185_credit_report_blocks.sql`, idempotent)
...
condition to `issue` and hard-blocks sign-off (`signOffGate` + the `db/184` trigger) until
...
- **Storage stays back-compatible:** keep `credit_reports.underwriting_finding` but store a
  **wrapper** `{ severity: <max of all>, types:[…], findings:[ {type, severity, code, message,
  ...
  top-level `severity`, **the existing `db/184` trigger and `signOffGate` keep gating with zero
  schema change**.
- **New migration `db/186_credit_findings_multi.sql`:** change the gate predicate from "top-level
...
- **P1 — Parse + store the blocks (E1).** Extend both parsers (`alerts[]`, reported DOB/residence,
  ...
  `db/185_credit_report_blocks.sql`, persist in `import.js` reusing `dbIdFor`. Tests: parser unit
...
- **P2 — Findings engine (E2).** Generalize `underwriting.js` to `collectFindings()` + the wrapper
  shape; `db/186_credit_findings_multi.sql` (gate iterates `findings[]`, per-finding reconcile,
