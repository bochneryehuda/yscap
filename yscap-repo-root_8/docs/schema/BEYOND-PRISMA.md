...
**960 objects**, and a database rebuilt from the Prisma
...
rebuild a database from them.** The 671 numbered migrations in `db/` (highest `db/675`) remain the only thing that builds this database.
...
- **cobrowse_sessions_open_idx** on `cobrowse_sessions`
- **cobrowse_sessions_watched_borrower_idx** on `cobrowse_sessions`
- **cobrowse_sessions_watched_staff_idx** on `cobrowse_sessions`
...
- **lt_loan_vendors_landlord_unremembered_idx** on `lt_loan_vendors`
...
- **cobrowse_sessions_control_status_chk** on `cobrowse_sessions`
- **cobrowse_sessions_kind_chk** on `cobrowse_sessions`
...
- **staff_view_sessions_not_self_chk** on `staff_view_sessions`
...
- **cobrowse_sessions** → `applications` — `FOREIGN KEY (application_id) REFERENCES applications(id) ON DELETE SET NULL`
- **cobrowse_sessions** → `staff_users` — `FOREIGN KEY (viewer_staff_id) REFERENCES staff_users(id) ON DELETE CASCADE`
- **cobrowse_sessions** → `borrowers` — `FOREIGN KEY (watched_borrower_id) REFERENCES borrowers(id) ON DELETE CASCADE`
- **cobrowse_sessions** → `staff_users` — `FOREIGN KEY (watched_staff_id) REFERENCES staff_users(id) ON DELETE CASCADE`
...
- **lt_borrower_landlords** → `borrowers` — `FOREIGN KEY (borrower_id) REFERENCES borrowers(id) ON DELETE SET NULL`
...
- **lt_pricer_investor_groups** → `staff_users` — `FOREIGN KEY (staff_id) REFERENCES staff_users(id) ON UPDATE CASCADE ON DELETE SET NULL`
- **lt_pricer_scenarios** → `staff_users` — `FOREIGN KEY (staff_id) REFERENCES staff_users(id) ON UPDATE CASCADE ON DELETE SET NULL`
...
- **staff_view_sessions** → `staff_users` — `FOREIGN KEY (staff_id) REFERENCES staff_users(id) ON DELETE CASCADE`
- **staff_view_sessions** → `staff_users` — `FOREIGN KEY (viewer_staff_id) REFERENCES staff_users(id) ON DELETE CASCADE`
...
- **track_record_verifications** → `track_records` — `FOREIGN KEY (track_record_id) REFERENCES track_records(id) ON DELETE CASCADE`
...
Every one of the 410 primary keys and 1421 indexes is
