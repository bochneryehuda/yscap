...
- **Retry + breaker** (`src/lib/ai/resilience.js`): full-jitter backoff honoring Azure's
  ...
  (`documentAi` block).
- **Analyze-once idempotency** (`db/203`, `fingerprint.js`): skip a paid re-read when the content
...
- **Clear-to-close gate**: `underwriting_review_cleared` — enforced in the app layer
  (`staff.js signOffGate`, `file-review.js fileFatalCount`) AND a DB trigger (`db/202`). The loan
...
- **One current extraction per (document, file)** — `db/204` unique partial index; two racing
