...
- **Retry + breaker** (`src/lib/ai/resilience.js`): full-jitter backoff honoring Azure's
  ...
  (`documentAi` block).
- **Analyze-once idempotency** (`db/180`, `fingerprint.js`): skip a paid re-read when the content
...
- **Clear-to-close gate**: `underwriting_review_cleared` — enforced in the app layer
  (`staff.js signOffGate`, `file-review.js fileFatalCount`) AND a DB trigger (`db/179`). The loan
...
- **One current extraction per (document, file)** — `db/181` unique partial index; two racing
...
`db/177` (extractions + findings + the gate condition), `db/178` (suggested actions + opens
condition), `db/179` (CTC guard trigger), `db/180` (idempotency columns), `db/181` (one-current-
