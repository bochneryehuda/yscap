...
1. **Foundation** ✅ (this PR) — firm + TPO identity + firm scoping + the curated
   API seam. `db/483` (tpo_firms + staff_users external flags + role CHECK +
   external-firm invariant), `db/484` (applications `is_tpo`/`tpo_firm_id`/
...
2. **Front door + firm onboarding** ✅ — internal admin surface (`admin-tpo.js`,
   ...
   `invite_tokens` + `/auth/accept` (`db/485` carries the firm + firm-admin flag +
...
5. **Orders + own-Xactus** — title / insurance / flood / credit on the TPO
   ...
   - **Phase 5b — own-Xactus (per-firm encrypted CREDIT credentials) ✅ (BUILT).**
     ...
     · **db/490** `tpo_firm_credit_credentials` (PK `tpo_firm_id`, `password_encrypted
...
6. **Appraisal view, draws, messaging + AE/AM "view as TPO"** — the remaining
   ...
   - **Phase 6c — AE/AM "view as TPO" (the impersonation feature) ✅ (BUILT).** An
     ...
     `db/486_tpo_view_sessions.sql`. Auth: the tpo-view impersonation block in
   ...
   - **Phase 6d — a broker ACCEPTS / DISPUTES a draw inspection result ✅ (BUILT).**
     ...
     {accept,dispute}` (db/487) runs the SAME server-side transitions as the borrower's
     ...
     not a token). Attributed: db/487 widens `accepted_via`/`disputed_via` to `'tpo'` +
   ...
   - **Phase 6e — broker ↔ our-team MESSAGING ✅ (BUILT).** Owner-approved scope:
     ...
     `conversations.kind`), never as `staff`. db/488 widens the three CHECKs to admit
     `'tpo'` IN-PLACE under each constraint's OWN name (the db/487 replay-idempotency
     ...
     the broker; db/488's DELETE removes any already-seated. `src/routes/tpo-chat.js`
