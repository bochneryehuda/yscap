...
| `test-lt-lp-field-fidelity.js` | A LIVE A/B PROBER, not a suite. For each field it prices TWO real scenarios differing in that field alone and compares the returned ladders, so it needs vendor credentials and makes two paid upstream calls per axis. Its offline half — `--offline`, which diffs the outgoing request bodies and needs no credentials — is the part that can prove a DROPPED field, and that half is exercised by `test-lt-lp-request-fields-pure.js` in the chain. The live half answers a question no offline test can (did the vendor PRICE it), and only a human should spend money asking it. |
