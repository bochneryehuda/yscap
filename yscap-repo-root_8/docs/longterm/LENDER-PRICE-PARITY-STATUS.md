...
- `apr` → *(no LP request path — engine facts only)* — an OPTIONAL number feeding the Layer-3 PPP high-cost prohibitions (`deephaven-ppp-matrix` `aprGt` rules, e.g. IL/NJ natural-person). APR is DERIVED (rate + fees), so it is a pure PASS-THROUGH: emitted into `lpScenarioToFacts` only when a scenario supplies one, and left `null` otherwise. The PPP matrix FAILS OPEN on a null apr (an `aprGt` rule requires a numeric apr), so an ordinary scenario is unchanged; only a scenario that already knows a high APR can trip the natural-person high-cost prohibition end-to-end. No value is ever invented. Accepted here so it is not 422'd as unsupported.
