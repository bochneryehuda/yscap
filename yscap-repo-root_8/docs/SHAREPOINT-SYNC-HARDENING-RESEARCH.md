...
| A7 | "SLO email named a doc as `last error: (not yet attempted)` — stuck 34.9h having NEVER been tried" (owner-reported 2026-07-17) | A document counted as *pending* but that the selection query (`pendingBatch`) never returned was **never attempted at all** — so there was no error to classify and no way to know why. Root: the mirror depended on ONE selection query; anything that query skipped (a `NULL`-`is_current` gap between `pendingBatch`'s exclusion and the settle pass; a poison-pill upload ahead of it in the batch; any unforeseen data condition) left a document invisible with attempts=0 forever. | (1) The enrichment SELECT is now SHARED (`ENRICH_SELECT`) and a document can be loaded by id (`enrichedRowById`) with NO filters. (2) `escalateStuckDocs()` **force-attempts** any stuck-but-never-tried doc through `forceAttemptDoc()` — a direct `mirrorRow` bypassing selection, with a 90s timeout (poison-pill safe) — so it either mirrors NOW or produces a REAL, classifiable error and a friendly card, instead of "not yet attempted" forever. (3) The regen/settle predicates are `NULL`-safe (`COALESCE(is_current, true)`), closing the select-vs-settle gap. A document can no longer sit un-attempted: within one escalation cycle it names its real reason. |
