-- 489_reassert_assignee_trigger.sql — re-assert the application_assignees
-- pointer-sync trigger AFTER db/484, so a boot-replay that skips db/392 can no
...
--   • db/484 (Phase 3 / TPO) widens that SAME-named constraint to also allow
...
--     migrate-boot's superseded-constraint detection recognizes it (db/484 re-adds
...
-- restored. The constraint itself is already left WIDE by db/484 (its wide re-add
