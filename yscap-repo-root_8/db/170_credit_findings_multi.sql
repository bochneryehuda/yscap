...
--
-- db/167 added credit_reports.underwriting_finding as a SINGLE finding object
-- {type,severity,...}; db/168 made a fatal one block completion of the credit
...
-- Same "latest IMPORTED report, id DESC tiebreak" selection as db/168 so the two
...
-- Trigger definition is unchanged from db/168, but re-assert it idempotently so a
...
-- funded/closed file is left alone. Mirrors db/168's backfill.
