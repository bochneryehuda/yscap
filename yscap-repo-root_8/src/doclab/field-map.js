...
const FIELDS = Object.freeze([
  ...
  { key: 'borrower_title', status: 'mapped', group: 'borrowers', array: 'borrowers',
    source: 'llc_borrowers.member_title / llc_members.member_title (db/509)',
  ...
  /* ─────────────────────────────── entity character ───────────────────────────────
     WAS THE BIGGEST GAP, AND IS NOW CLOSED (owner-directed 2026-08-09, db/509).
     ...
     re-derive one of these from a string comparison here. */
  { key: 'type_of_organization', status: 'derived', group: 'entity',
    source: 'llcs.entity_type (+ entity_subtype) via lib/entity-type.docLabVariables',
    note: 'The whole entity block hangs off this one answer. The back book was stamped `llc` without anybody choosing it (entity_type_confirmed = false), so the payload warns when the type on a file was assumed rather than stated. A PARTNERSHIP is refined by its sub-kind — "general partnership" and "limited partnership" are different legal entities with different liability, and this word is printed verbatim onto a recorded instrument. A TRUST is deliberately NOT refined: "trust" is never wrong, and the trust\'s own name carries the rest ("The Smith Family Trust, dated March 3, 2019").' },
  { key: 'acknowledgement_corporate_status', status: 'derived', group: 'entity',
    source: 'llcs.entity_type via lib/entity-type.docLabVariables',
    note: 'OPEN QUESTION FOR PLL. DocLab publishes wording for an LLC and a corporation ONLY — and publishes it three different ways: the dictionary says "operating agreement and its members", the integration document says \'Enter "Bylaws and Board of Directors" or "Operating Agreement and Members"\', and their own master payload sends "operating agreement and members ". There is NO published wording for a partnership or a trust, so ours ("partnership agreement and its partners" / "trust agreement and its trustees") is an analogy, not their text. It merges into a sentence on a recorded instrument, so confirm it with PLL before going live. Overridable per file via `acknowledgementCorporateStatus`.' },
  { key: 'bylaws_operating_agreement', status: 'derived', group: 'entity',
    source: 'llcs.entity_type via lib/entity-type.docLabVariables',
    note: 'Must be exactly one of DocLab\'s permitted strings ("bylaws" / "operating agreement" / "partnership agreement" / "trust agreement").' },
  { key: 'operating_agreement_or_bylaws', status: 'derived', group: 'entity',
    source: 'llcs.entity_type via lib/entity-type.docLabVariables',
    note: 'Same value, second variable name. DocLab carries both, so both are sent.' },
  { key: 'membership_interest_percentage', status: 'mapped', group: 'entity',
    ...
    note: 'ONLY sent for an entity whose owners hold a percentage — an LLC, a PARTNERSHIP and a TRUST all do. A corporation is sent number_of_shares instead; sending both would print whichever the template asks for, so the wrong one would land on a pledge as a fact.' },
  { key: 'number_of_shares', status: 'mapped', group: 'entity',
    source: 'llc_borrowers.shares / llc_members.shares (per owner, db/509)',
  ...
  { key: 'certificate_number', status: 'mapped', group: 'entity',
    source: 'llc_borrowers.certificate_number / llc_members.certificate_number (db/509)',
  ...
  { key: 'trustee', status: 'needs_rule', group: 'third_parties', source: null,
    note: 'A FOURTH FALSE FRIEND, alongside "lender", "underwriter" and "state_abbrev": this is the trustee under the DEED OF TRUST — DocLab\'s own words, "most often the title company at closing" — and it has NOTHING to do with the trustee of a borrowing TRUST. Wiring a trust\'s trustee here would name the borrower\'s own family member as the party holding legal title to the property for the lender. The trust\'s trustees are people on the entity, and reach the documents as signatories with the title "Trustee". Deed-of-trust states only (3 RTL templates, all DOT); some states require a named local trustee, which is a legal designation, so never auto-fill it from the title contact either.' },
