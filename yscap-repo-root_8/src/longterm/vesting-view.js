'use strict';
/**
 * HOW A LONG-TERM LOAN VESTS, AS A SCREEN SHOWS IT — the one answer, so two screens
 * cannot give two.
 *
 * ⚠️ THERE ARE TWO VESTING MODULES AND THEY DO DIFFERENT JOBS. Read this before
 * touching either, because they were once ONE FILE and that broke the loan reader:
 *
 *   · `vesting.js`      — the READ-TIME rule. It is handed the raw Encompass field
 *                         map (`{4008, 1859}`) by `sync/loans.js` and decides what to
 *                         WRITE into `lt_loans.vesting_type` / `vesting_entity_name`.
 *                         It owns `FIELD_IDS`, which that sync spreads into its
 *                         fieldReader batch.
 *   · THIS FILE         — the DISPLAY-TIME rule. It is handed the STORED loan row
 *                         those columns live on, plus the 1003's party rows, and
 *                         decides the ONE answer every long-term surface shows.
 *
 * So this file is not a second opinion about field 4008 — it CONSUMES what that
 * rule already wrote, and adds the only thing it cannot know: what the 1003 says.
 *
