...
const UNMEASURED = [
  ...
  'THE HOLDBACK IS NOT YET APPLIED TO THE PRICE BY THE ENGINE. quote.js carries holdbackMilli in the reconstruction record and deliberately does NOT subtract it (its own comment: a money rule that needs the owner\'s exact formula). Our composed price is post-holdback today only because the sibling sheet\'s BASE LADDER is the LP-measured one. The owner has now given the relationship (LP = sheet − holdback), but wiring it into pricing.priceRung moves every price on every program and is a shared-engine change that belongs in its own commit, not in this sheet slice. Until then the FRAME INVARIANT in the header is what keeps the cap and the price in step.',
