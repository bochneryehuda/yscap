...
async function conditionsForLoan(loanId, opts = {}) {
  ...
  const docs = rows.length
    ? (await db.query(
      `SELECT l.condition_id, d.id, d.title, d.title_with_index, d.status,
              d.milestone_name, d.attachment_count, d.encompass_created_at
  ...
  for (const d of docs) {
    ...
    docsByCondition.get(d.condition_id).push({
      id: d.id,
...
async function documentsForLoan(loanId, opts = {}) {
  ...
  const { rows } = await db.query(
    `SELECT d.id, d.encompass_document_id, d.title, d.title_with_index, d.status,
            d.milestone_name, d.application_name, d.attachment_count,
...
module.exports = {
  ...
  documentsForLoan,
  syncStamps,
  _internals: { conditionRank, groupOf, documentOutstanding, describeCondition, safeText, DONE_STATUSES },
};
