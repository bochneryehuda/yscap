...
// table). That is a drift trap: every field added to FIELD_MAP had to ALSO be
// remembered here, and three times it was NOT — `lender` (the note buyer, made
// bidirectional 2026-07-20), `desired_rate` (2026-07, "register mirrors the rate
// into Desired Rate"), and `actual_closing` (made bidirectional 2026-08-11). Each
// silently failed to PUSH to ClickUp (buildTaskFields read `undefined`) AND showed
// as "—" on the ClickUp compare table even when the DB held a value — the exact
// owner report (2026-08-11): "I changed the lender and pushed updates but it didn't
// update in ClickUp, and our system also doesn't have a lender name."
//
// These build the context FROM the field map instead, so adding a field to
// FIELD_MAP is now enough — the context can never drift from it again. Pure (no DB);
// the async/special columns (subject + current address geocoding, the decrypted SSN)
// are layered on by loadPushContext.

// The two b-table columns the SELECT ALIASES to avoid ambiguity with `a.*`.
const BORROWER_COL_ALIAS = { email: 'b_email', fico: 'b_fico' };

/** The application-side mapper context, built from every t:'a' FIELD_MAP column. */
function appContextFromRow(row) {
  const app = {};
  for (const f of mapper.FIELD_MAP) {
    if (f.t === 'a' && f.col && Object.prototype.hasOwnProperty.call(row, f.col)) app[f.col] = row[f.col];
  }
  // The ClickUp-owned TASK status is not a mapped custom field (it becomes the
  // create status), so it is not in FIELD_MAP — carry it explicitly.
  app.internal_status = row.internal_status || null;
  return app;
}

/** The borrower-side mapper context, built from every t:'b' FIELD_MAP column.
 *  The name parts + full_name, marital status, SSN and current address are
 *  specials (not plain mapped columns) and are layered on by the caller. */
function borrowerContextFromRow(row) {
  const b = {};
  for (const f of mapper.FIELD_MAP) {
    if (f.t !== 'b' || !f.col) continue;
    const rowKey = BORROWER_COL_ALIAS[f.col] || f.col;
    if (Object.prototype.hasOwnProperty.call(row, rowKey)) b[f.col] = row[rowKey];
  }
  return b;
}
...
async function loadPushContext(appId) {
  ...
  // Build the mapper context FROM the field map (appContextFromRow /
  // borrowerContextFromRow) so it can never silently drop a mapped field again —
  // then layer on the specials the field map doesn't carry: the geocoded subject +
  ...
  // generated by Postgres from the four parts). The mapper writes `full_name` into
  // ClickUp's single "Borrower Name" field, so the card always shows exactly the
  // whole name PILOT holds.
  borrowerCtx.middle_name = row.middle_name;
  borrowerCtx.name_suffix = row.name_suffix;
  borrowerCtx.full_name = row.b_full_name;
  borrowerCtx.marital_status = row.marital_status;
  borrowerCtx.ssn = ssn;
  borrowerCtx.current_address = await withCoords(row.current_address);

  const ctx = {
    app: appCtx,
    borrower: borrowerCtx,
