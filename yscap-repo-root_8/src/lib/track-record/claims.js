 ...
 */
async function claimForReview(list, staffId, client) {
  const db = client || require('../../db');
  const wanted = ids(list);
  const out = { claimed: [], heldByOthers: [] };
  if (!wanted.length || !staffId) return out;
  ...
  try {
    const taken = (await db.query(
      `UPDATE track_record_candidates c
        ...
        RETURNING c.id`,
      [wanted, staffId])).rows.map((r) => String(r.id));
    out.claimed = taken;
 ...
 */
async function releaseClaims(list, staffId, client) {
  const db = client || require('../../db');
  const wanted = ids(list);
  if (!wanted.length || !staffId) return 0;
  try {
    const r = await db.query(
      `UPDATE track_record_candidates
          SET claimed_by = NULL, claimed_at = NULL
        WHERE id::text = ANY($1::text[]) AND claimed_by = $2`,
      [wanted, staffId]);
    return r.rowCount || 0;
  } catch (_) { return 0; }
}
