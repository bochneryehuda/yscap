...
async function processReceivedEvent(event) {
  ...
  // Returned documents (#orders): save the vendor's attachments back onto the
  // order(s) as UNASSIGNED documents for the team to classify. Best-effort — the
  // reply still forwards to the team regardless. Retrieved once and reused.
  if (orderRefs.length && Array.isArray(full.attachments) && full.attachments.length && !isAutoGenerated(full)) {
    try {
