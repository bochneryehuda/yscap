...
async function processReceivedEvent(event) {
  ...
  // per-file loop, where the same module decides whether it really is the link; it
  // never forwards a vendor email as a plain file reply (the forward runs only for a
  // human sender — see payment-link-inbox.handleInbound's `handled:false` paths),
  // and a ping-pong is impossible because the vendor's address is never emailed.
