    ...
    for (const ref of pending) {
      try {
        ...
        if (res.failedTransient) {
          console.warn(`[closing-inbox] ${res.failedTransient} closing attachment(s) for ${ref.applicationId} hit a storage/database failure — leaving the chain unmarked so a redelivery retries them (already-filed documents are matched by content hash and will not duplicate).`);
        } else {
