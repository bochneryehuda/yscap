...
async function processReceivedEvent(event) {
  ...
  //
  // EXCEPT the one machine-sent email a file mailbox is waiting for: Class Valuation's
  // payment link (src/class/payment-link-inbox.js). It is transactional mail from a
  // system, and whether their sender stamps `Precedence: bulk` / `Auto-Submitted` is
  // not something we control — filed as an auto-reply it would never reach the
  // borrower and the order card would say "not received yet" forever (pre-merge audit
  // round 2). So a vendor sender on a file that expects a link is let through to the
  // per-file loop, where the same module decides whether it really is the link. A
  // machine-stamped vendor email that turns out NOT to be the link (a receipt, a
  // bounce) is filed there as the auto-reply it is, never forwarded as a plain file
  // reply; and a ping-pong is impossible because the vendor's address is never emailed.
  let vendorLinkExpected = false;
  if (fileAddrIds.size) {
    try {
      const plinkMod = require('../class/payment-link-inbox');
      if (plinkMod.isVendorSender(fromEmail)) {
        for (const id of fileAddrIds) { if (await plinkMod.orderExpectingLink(db, id)) { vendorLinkExpected = true; break; } }
      }
    } catch (_) { vendorLinkExpected = false; }
  }
  if (isAutoGenerated(full) && !vendorLinkExpected) {
  ...
  for (const applicationId of applicationIds) {
    ...
    if (fileAddrIds.has(applicationId)) {
      ...
      if (plink && plink.handled) {
        ...
        anyForwarded = true;
        // The per-file history index and the activity feed read this column; the
        // ordinary forward stamps it further down, which this path never reaches. The
        // order lookup inside handleInbound already proved the file exists.
      ...
      // The vendor's email was let past the auto-reply gate only so the module above
      // could look at it. Not the link, and machine-stamped: file it as the auto-reply
      // it is — a "payment successful" note or a bounce is not a team forward
      // (post-merge audit 2026-09-03). A vendor email a person wrote still forwards.
      if (plink && !plink.handled && isAutoGenerated(full)) {
        // Said out loud: if Class ever words its link so that it is refused here, this
        // line is the only trace beside an order card that says "not received yet".
        console.warn(`[inbound-file-email] vendor email on file ${applicationId}, which expects a payment link, filed as an auto-reply (${plink.reason || 'not the link'}): "${String(subject || '').slice(0, 80)}"`);
        appResults[applicationId] = 'auto_reply'; lastTerminal = lastTerminal || 'auto_reply'; continue;
      }
    }
