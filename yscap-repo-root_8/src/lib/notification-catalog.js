...
const CATALOG = [
  ...
  // ─── Policy exceptions (the loan_exceptions register — redesign 2026-07-24;
  //     previously these types existed but were absent from this catalog) ─────
  { key: 'guaranty_exception',    label: 'Guaranty waiver requested (to super-admins)',
    description: "A request to waive a co-borrower's personal guaranty is waiting in the Exceptions box.",
    category: 'conditions', audience: 'admin', forced: true, default_mode: 'automatic' },
  { key: 'guaranty_exception_decided', label: 'Guaranty waiver decided',
    description: "The super-admin's approve/deny on a guaranty waiver, back to the file's team.",
    category: 'conditions', audience: 'staff', forced: true, default_mode: 'automatic' },
  { key: 'guaranty_exception_comment', label: 'Comment on a guaranty-waiver exception',
    description: "A participant commented on a guaranty-waiver exception thread.",
    category: 'conditions', audience: 'staff', forced: false, default_mode: 'automatic' },
  { key: 'esign_before_ctc_exception', label: 'Send-before-clear-to-close requested (to super-admins)',
    description: "A request to send the term-sheet package before every send requirement is met, waiting in the Exceptions box.",
    category: 'conditions', audience: 'admin', forced: true, default_mode: 'automatic' },
  { key: 'esign_before_ctc_exception_decided', label: 'Send-before-clear-to-close decided',
    description: "The super-admin's decision on an early-send exception — names exactly what was waived and what still applies.",
    category: 'conditions', audience: 'staff', forced: true, default_mode: 'automatic' },
  { key: 'pricing_exception',     label: 'Pricing exception requested (to super-admins)',
    description: "A pricing/guideline exception request (from staff or the borrower) is waiting in the Exceptions box.",
    category: 'conditions', audience: 'admin', forced: true, default_mode: 'automatic' },
  { key: 'pricing_exception_decided', label: 'Pricing exception decided',
    description: "The super-admin's approve/deny on a pricing exception, back to the file's team.",
    category: 'conditions', audience: 'staff', forced: true, default_mode: 'automatic' },
  ...
  { key: 'exception_expired',     label: 'Approved exception expired',
    description: "An approved, time-boxed exception passed its validity date and no longer grants anything.",
    category: 'conditions', audience: 'staff', forced: false, default_mode: 'automatic' },
  ...
  { key: 'draw_message',          label: 'Coordinator message from the draw desk',
    ...
    category: 'draws', audience: 'borrower', forced: false, default_mode: 'automatic' },
  { key: 'draw_inbound',          label: 'Draw submitted for review (from Sitewire)',
    description: "The borrower actually submitted a draw for review, or an inspector moved it along — picked up by the reconcile pass.",
    category: 'draws', audience: 'staff', forced: false, default_mode: 'automatic' },
  { key: 'draw_draft_started',    label: 'Borrower started a draw (draft only)',
    description: "The borrower pressed Start in Sitewire and is filling the draw in. It is a DRAFT — nothing has been submitted and nothing is needed from the desk. In-app only by default, so it never reads as a real request.",
    category: 'draws', audience: 'staff', forced: false, default_mode: 'automatic' },
  ...
  { key: 'workflow_returned',     label: 'Workflow item returned',
    ...
    category: 'status', audience: 'staff', forced: false, default_mode: 'automatic' },
  { key: 'workflow_picked_up',     label: 'Your file was picked up by the processor',
    description: "The processor picked your file up and is now working on it (owner-directed 2026-09-01).",
    category: 'status', audience: 'staff', forced: false, default_mode: 'automatic' },
