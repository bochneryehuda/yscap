 ...
 * happens automatically in the DB trigger (db/174) when the frozen score lands
...
async function persistImport({ reportRowId, applicationId, actorId, providerId, parsed, scored, assessment, rawXml, orderMeta }) {
  ...
  try {
    ...
    // (signOffGate + the db/186 trigger) until reconciled. Identity mismatches are
    ...
    if (assessment.decision === 'imported') {
      ...
      // reopen trigger (db/174) is AFTER ROW and reads GREATEST(primary.fico,
