 ...
 * happens automatically in the DB trigger (db/177) when the frozen score lands
...
async function persistImport({ reportRowId, applicationId, actorId, providerId, parsed, scored, assessment, rawXml, orderMeta }) {
  ...
  try {
    ...
    // (signOffGate + the db/189 trigger) until reconciled. Identity mismatches are
    ...
    if (assessment.decision === 'imported') {
      ...
      // reopen trigger (db/177) is AFTER ROW and reads GREATEST(primary.fico,
