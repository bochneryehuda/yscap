  ...
  try {
    ...
    const a = await client.query(
      ...
      [borrowerId, officerId, officerName, p.program || p.dealType || null, F.sanitizeLoanType(p.loanType || p.purpose),   // #95: public form can't persist a program as a loan type
       /* jsonbText, not JSON.stringify — a NUL byte anywhere in a PUBLIC form
          post is refused by Postgres and would strand the lead with a 500, the
          same failure mode as an unstorable number above (audit 2026-08-02). */
