...
router.post('/:appId/documents/:documentId/analyze', async (req, res, next) => {
  try {
    ...
    // IDEMPOTENCY (db/199): if this exact document (same content hash) was already analyzed as
    ...
    // there IS a condition for signOffGate (+ the db/198 trigger) to hold until it's resolved —
