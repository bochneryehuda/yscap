...
router.post('/:appId/documents/:documentId/analyze', async (req, res, next) => {
  try {
    ...
    // IDEMPOTENCY (db/186): if this exact document (same content hash) was already analyzed as
    ...
    // there IS a condition for signOffGate (+ the db/185 trigger) to hold until it's resolved —
