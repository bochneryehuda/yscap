...
router.get('/chat/conversations', async (req, res) => {
  ...
  await db.query(`
    ...
    SELECT a.id, v.kind, CASE WHEN v.kind='borrower'
     ...
     WHERE a.deleted_at IS NULL ${scoped ? `AND ${visibleOfficersSql('a', '$1')}` : ''}
  ...
  await db.query(`
    ...
    SELECT c.id, p.kind, p.id, p.label,
     ...
     WHERE a.deleted_at IS NULL AND c.kind NOT IN ('custom','tpo') AND p.id IS NOT NULL
       ...
       -- inbox load would re-seat the broker (undoing db/528's cleanup) and email them our
       ...
       ${scoped ? `AND ${visibleOfficersSql('a', '$1')}` : ''}
  ...
  const r = await db.query(
    `SELECT c.id, c.application_id, c.kind, c.name, c.emoji, c.topic, c.borrower_visible, c.archived_at,
      ...
      WHERE a.deleted_at IS NULL AND c.archived_at IS NULL
        ${scoped ? `AND (cm.conversation_id IS NOT NULL OR ${visibleOfficersSql('a', '$1')})` : ''}
  ...
  const r = await db.query(
    `SELECT m.id, m.seq, m.conversation_id, m.body, m.created_at, m.attachment_kind,
      ...
      WHERE m.deleted_at IS NULL AND m.body ILIKE $1 AND a.deleted_at IS NULL
        ${scoped ? `AND (cm.conversation_id IS NOT NULL OR ${visibleOfficersSql('a', '$2')})` : ''}
        ${convFilter}
