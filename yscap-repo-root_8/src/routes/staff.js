...
async function signOffGate(itemId, actor) {
  ...
  // db/187 so no path can bypass it.
  if (isCredit) {
    ...
    // shared engine helper, so the app layer and the db/189 trigger always agree.
    //
    ...
    // an earlier fatal. The db/191 trigger applies the SAME rule over the same rows.
