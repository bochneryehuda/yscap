...
router.post('/conversations/:cid/read', async (req, res) => {
  ...
  // Inside a BORROWER VIEW the staffer is looking at the borrower's thread — and
  // likewise a borrower's ASSISTANT (#15) — they must not consume the borrower's
  // unread state on their behalf (same rule as the legacy /messages read stamp
  // in borrower.js). The thread renders identically; only the receipt write is
  // skipped.
  if (req.impersonation) return res.json({ ok: true, skipped: 'borrower_view' });
  if (req.assistant) return res.json({ ok: true, skipped: 'assistant' });
