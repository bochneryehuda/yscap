-- Integration test for db/184: the fatal FICO-mismatch finding is a HARD gate on
...
BEGIN
  ...
  -- ==== db/186: the GENERALIZED findings[] wrapper (E2) ====================
  ...
  -- ==== db/187: a FATAL ALERT on a REVIEW report must also block ============
  ...
  -- ==== db/188: a REVIEW fatal must not be masked by a newer NULL REVIEW =====
  DELETE FROM credit_reports WHERE application_id=aid;
