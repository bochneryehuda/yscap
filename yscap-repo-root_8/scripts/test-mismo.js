...
assert(xml.includes('MISMOReferenceModelIdentifier="3.4.032420160128"'), 'has reference model id');
...
assert.strictEqual(parsed.loan.loanType, 'Refinance — Cash-Out', 'cash-out loan type restored via detail');
...
// Divorced collapses to MISMO's Unmarried bucket but is restored exactly from
// the lender EXTENSION — the round-trip must NOT silently turn it into Single.
assert.strictEqual(parsed.borrower.maritalStatus, 'Married', 'borrower marital exact');
assert.strictEqual(parsed.coBorrower.maritalStatus, 'Divorced', 'co-borrower Divorced preserved (not Single)');
assert.strictEqual(parsed.property.propertyType, 'Multi 2-4', 'property type preserved via extension');
ok('co-borrower fields + exact marital + property type round-trip');
...
// ------------------------------------ 3b) role-based party classification
// A realistic file: the borrower, a LoanOriginator individual, and a
// LoanOriginationCompany (lender) entity — mirrors real GSE/iLAD output. The
// loan originator must NOT become the co-borrower, and the lender must NOT
// become the vesting LLC.
const withParties = `<?xml version="1.0"?>
<MESSAGE xmlns="http://www.mismo.org/residential/2009/schemas" xmlns:xlink="http://www.w3.org/1999/xlink">
  <DEAL_SETS><DEAL_SET><DEALS><DEAL>
    <LOANS><LOAN LoanRoleType="SubjectLoan"><TERMS_OF_LOAN><BaseLoanAmount>356000</BaseLoanAmount><LoanPurposeType>Purchase</LoanPurposeType></TERMS_OF_LOAN></LOAN></LOANS>
    <PARTIES>
      <PARTY><INDIVIDUAL><NAME><FirstName>Simcha</FirstName><LastName>Kramer</LastName></NAME></INDIVIDUAL>
        <ROLES><ROLE><BORROWER/><ROLE_DETAIL><PartyRoleType>Borrower</PartyRoleType></ROLE_DETAIL></ROLE></ROLES>
        <TAXPAYER_IDENTIFIERS><TAXPAYER_IDENTIFIER><TaxpayerIdentifierType>SocialSecurityNumber</TaxpayerIdentifierType><TaxpayerIdentifierValue>111223333</TaxpayerIdentifierValue></TAXPAYER_IDENTIFIER></TAXPAYER_IDENTIFIERS>
      </PARTY>
      <PARTY><LEGAL_ENTITY><LEGAL_ENTITY_DETAIL><FullName>YS Capital Group</FullName></LEGAL_ENTITY_DETAIL></LEGAL_ENTITY>
        <ROLES><ROLE><ROLE_DETAIL><PartyRoleType>LoanOriginationCompany</PartyRoleType></ROLE_DETAIL></ROLE></ROLES>
      </PARTY>
      <PARTY><INDIVIDUAL><NAME><FirstName>Solomon</FirstName><LastName>Weiss</LastName></NAME></INDIVIDUAL>
        <ROLES><ROLE><ROLE_DETAIL><PartyRoleType>LoanOriginator</PartyRoleType></ROLE_DETAIL></ROLE></ROLES>
      </PARTY>
    </PARTIES>
  </DEAL></DEALS></DEAL_SET></DEAL_SETS>
</MESSAGE>`;
const rp = parseMismoXml(withParties);
assert.strictEqual(rp.borrower.firstName, 'Simcha', 'the real borrower is imported');
assert.strictEqual(rp.borrower.ssn, '111223333', 'the borrower SSN belongs to the borrower');
assert.strictEqual(rp.coBorrower, null, 'the LOAN ORIGINATOR is NOT imported as a co-borrower');
assert.strictEqual(rp.llc, null, 'the LENDER entity is NOT imported as the vesting LLC');
ok('role-based classification: originator/lender are not mistaken for borrower/LLC');

// A genuine vesting entity (TitleHolder) IS imported as the LLC.
const withVesting = withParties.replace('LoanOriginationCompany', 'TitleHolder');
const rv = parseMismoXml(withVesting);
assert.strictEqual(rv.llc && rv.llc.name, 'YS Capital Group', 'a TitleHolder entity IS imported as the vesting LLC');
ok('a title-holding entity is correctly imported as the vesting LLC');
...
assert.strictEqual(E.fromMismoLoanPurpose('Refinance', 'CashOut'), 'Refinance — Cash-Out', 'loan purpose reverse w/ cashout');
assert.strictEqual(E.fromMismoLoanPurpose('Refinance', 'NoCashOut'), 'Refinance — Rate & Term', 'rate&term reverse');
assert.strictEqual(E.fromMismoLoanPurpose('Construction'), null, 'construction is not a portal loan_type -> null');
assert.strictEqual(E.toMismoLoanPurpose('Refinance — Cash-Out'), 'Refinance', 'real refi vocab maps to Refinance (not Other)');
assert.strictEqual(E.unitsHint('SFR (1 unit)'), 1, 'unit-annotated SFR -> 1 unit');
assert.strictEqual(E.unitsHint('Multi 2–4'), 2, 'en-dash Multi 2–4 -> 2 units');
ok('enum crosswalk maps both directions (real vocab)');
