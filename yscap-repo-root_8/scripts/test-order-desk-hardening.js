...
(async () => {
  ...
  console.log('\nB. the borrower can never download what the vendor sent us');
  {
    ...
    // PREVIOUS AND FUTURE: db/453 must fix what is already on disk. Put a row back
    ...
    const sql = require('fs').readFileSync(require('path').join(__dirname, '..', 'db', '453_order_returns_staff_only.sql'), 'utf8');
    await db.query(sql);
    ...
    assert(healed.visibility === 'staff_only' && healed.source_type === 'system',
      'db/453 closes the exposure on a document that was already filed');
