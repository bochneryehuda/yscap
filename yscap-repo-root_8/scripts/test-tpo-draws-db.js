...
(async () => {
  ...
  try {
    ...
    // ── db/479 REPLAY idempotency ──
    // Every migration re-runs on every boot, and by now a 'tpo' accepted + disputed row exists.
    // db/479 widens the accepted_via/disputed_via CHECKs UNDER db/454's / db/193's OWN names, so when
    // those earlier files replay (BEFORE db/479 in numeric order) their `IF NOT EXISTS(<own name>)`
    ...
    for (const mig of ['193_draw_findings_public.sql', '454_investor_draw_delivery.sql', '479_tpo_draw_actions.sql']) {
      let threw = null;
