...
const sql = fs.readFileSync(path.join(__dirname, '..', 'db', '400_entity_adoption.sql'), 'utf8');
assert.ok(sql.includes(`'${ea.ENTITY_DOCS_TEMPLATE_CODE}'`), 'db/400 seeds the entity-documents template');
assert.ok(/'application', 'both', 'document'/.test(sql), 'and seeds it application-scoped so it can be posted on a file');
assert.ok(/adopted_from_application_id/.test(sql) && /adopted_at/.test(sql), 'db/400 adds the adoption stamp the liquidity guard reads');
