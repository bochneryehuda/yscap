 ...
 * (owner-directed 2026-08-02; db/396).
 *
...
(async () => {
  ...
  try {
    ...
    assert(!!ssnTpl, 'db/396 seeded the SSN-verification template');
    ...
    assert(emdTpl2.label === 'Earnest money deposit (EMD) verification'
      ...
      'the template wording SURVIVES a second boot (db/191 re-asserts the old borrower_label; db/396 runs after it)');
