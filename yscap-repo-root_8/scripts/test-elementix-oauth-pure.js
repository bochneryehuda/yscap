...
(async () => {
  ...
  /* THE EXACT LIST, on purpose — a switch appearing here is a deliberate act.
     ...
     `ELEMENTIX_HISTORY_IMPORT_ENABLED` is the fourth, added the same day (the
     deep-import build's pre-merge audit): it governs the back-book sweep that
     builds borrowers' track records + entities from their cached Elementix
     profiles. Same reason it is a switch — it writes onto real borrower
     profiles, so a human must be able to stop it at runtime. */
  assert.deepStrictEqual(elx.sort(),
    ['ELEMENTIX_CRM_SYNC_ENABLED', 'ELEMENTIX_DRYRUN', 'ELEMENTIX_ENABLED',
      'ELEMENTIX_HISTORY_IMPORT_ENABLED']);
